...
module.exports = {
	name: "taddrole",
    description: "Adds users with a specified role to a ticket.",
    usage: "[reason]",
    cooldown: 4,
    ...
    execute(message) {
        ...
        message.guild.channels.cache.find(chan => chan.name === `ticket-${message.author.username.toLowerCase()}`)
                        ...
                        fetchedChannel.updateOverwrite(therole.id, { VIEW_CHANNEL: true });
                        const welcomeChannelEmbed = new Discord.MessageEmbed()
                            .setColor(0x03c6fc)
                            .setDescription(`👋   **The role has been added to this ticket.**`)
